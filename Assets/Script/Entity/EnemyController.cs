...
public class EnemyController : BaseController
{
    ...
    protected override void HandleAction()
    {
        ...
        if (distance <= followRange)    // 타겟이 일정 거리 안에 있을 때만 추적 시작
        {
            ...
            if (distance <= weaponHandler.AttackRange)
            {
                ...
                RaycastHit2D hit = Physics2D.Raycast(transform.position, direction, weaponHandler.AttackRange * 1.5f, // 사거리보다 약간 여유 있게
    ...
    protected Vector2 DirectionToTarget()
    {
        if (target == null)
        {
            ...
            return Vector2.zero; // 대상이 없으면 이동 방향을 정지
        }
    ...
    public override void Death()
    {
        ...
        {
            Debug.LogWarning("EnemyManager is null while handling enemy death.");
        }
    }
